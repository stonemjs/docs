    ...
    if (inlineSelector) {
        useEventListener('dblclick', (event) => {
            ...
            if (enabled.value && el.matches(inlineSelector)) {
                ...
                (message ??= new Message()).pop(`${CHECK_ICON}<span>${locale.value.copied} </span>`, duration);
            }
        }, { passive: true });
    }
};
