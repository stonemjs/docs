import { checkGithubUsername, digestSHA256, getContributorInfo, getUserNameWithNoreplyEmail, } from './utils/index.js';
export const getRawContributors = (commits, options, gitProvider) => {
    ...
    for (const commit of [...commits].reverse()) {
        ...
        for (const { name, email } of authors) {
            ...
            const config = getContributorInfo({ name: usernameWithNoreplyEmail ?? name, email }, options.info);
...
export const resolveContributors = (commits, gitProvider, options, extraContributors = []) => {
    ...
    if (options.info?.length && extraContributors.length) {
        for (const extraContributor of extraContributors) {
            if (contributors.every((item) => item.username !== extraContributor &&
                item.name !== extraContributor)) {
                const contributorInfo = getContributorInfo({ name: extraContributor }, options.info);
                if (!contributorInfo)
                    continue;
                const result = {
