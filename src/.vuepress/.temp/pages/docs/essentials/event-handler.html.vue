...
This minimal contract allows Stone.js to remain completely adaptable, from tiny edge-deployed functions to complex applications with routers and dynamic flows.</p>
<h3 id="the-continuum-perspective" tabindex="-1"><a class="header-anchor" href="#the-continuum-perspective"><span>The Continuum Perspective</span></a></h3>
<p>In the <strong>Continuum Architecture</strong>, context and domain form a dynamic relationship.<br>
Your domain should never assume full control over its context—nor should it ignore it completely. This tension is captured in the <strong>uncertainty principle</strong>:</p>
<blockquote>
<p><em>“The domain cannot both fully know its execution context and remain independent from it.”</em></p>
</blockquote>
<p>The event handler embodies this principle.<br>
It accepts an <a href="./incoming-event"><code v-pre>IncomingEvent</code></a>, reacts with domain logic, and produces an outcome—without ever binding itself to the raw source of that event.<br>
It doesn’t care whether the event came from HTTP, WebSocket, or a CLI—it just handles <em>intentions</em>, not mechanics.</p>
<h3 id="where-it-lives" tabindex="-1"><a class="header-anchor" href="#where-it-lives"><span>Where It Lives</span></a></h3>
<p>The event handler belongs to the <strong>initialization dimension</strong> of Stone.js.<br>
It runs after your app is bootstrapped and ready to process a request.<br>
Every application <strong>must</strong> define at least one event handler—or it cannot react to incoming events at all.</p>
<Mermaid id="mermaid-29" code=""></Mermaid><p>By default, a Stone.js app has one event handler. If you need multiple entry points, use the <a href="../router">Router</a> to delegate the right events to the right handlers.</p>
<h2 id="variants-and-execution" tabindex="-1"><a class="header-anchor" href="#variants-and-execution"><span>Variants and Execution</span></a></h2>
<p>Stone.js supports <strong>three variants</strong> for defining event handlers:</p>
<ul>
...
<p>No matter the variant, the contract remains the same:<br>
You receive an <code v-pre>IncomingEvent</code> and optionally return a response.</p>
<Tabs id="59" :data='[{"id":"Class-based"},{"id":"Function-based"},{"id":"Function-based"}]' :active="0" tab-id="class-factory-function">
...
<h3 id="class-based-event-handler" tabindex="-1"><a class="header-anchor" href="#class-based-event-handler"><span>Class-based Event Handler</span></a></h3>
<p>If you prefer structure and decorators, this is the default variant.</p>
<p>Just implement the <code v-pre>handle</code> method from the <code v-pre>IEventHandler</code> interface:</p>
<div class="language-typescript line-numbers-mode" data-highlighter="prismjs" data-ext="ts"><pre v-pre><code><span class="line"><span class="token keyword">import</span> <span class="token punctuation">{</span> IncomingEvent<span class="token punctuation">,</span> IEventHandler <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">"@stone-js/core"</span></span>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>This shape integrates seamlessly with the declarative API.</p>
</template>
<template #tab1="{ value, isActive }">
<h3 id="function-based-event-handler" tabindex="-1"><a class="header-anchor" href="#function-based-event-handler"><span>Function-based Event Handler</span></a></h3>
<p>For fast prototyping or single-purpose apps, you can write a simple function:</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Great for edge functions, CLI tools, or single-route logic.</p>
</template>
<template #tab2="{ value, isActive }">
<h3 id="factory-based-event-handler" tabindex="-1"><a class="header-anchor" href="#factory-based-event-handler"><span>Factory-based Event Handler</span></a></h3>
<p>Need access to the service container or runtime dependencies?<br>
Use a factory-based handler: a function that returns your actual handler function.</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>This gives you full access to registered services at runtime, while keeping the handler itself focused.</p>
</template>
</Tabs>
<p>Each variant is valid and interchangeable.<br>
The choice depends on your app size, your preferred programming style, and how much framework-level automation you want.</p>
...
<p>This consistency ensures that:</p>
<ul>
<li>Your domain logic remains isolated from transport concerns</li>
<li>Your application remains portable and platform-agnostic</li>
<li>You always deal with the same <a href="./incoming-event"><code v-pre>IncomingEvent</code></a> interface or its subclasses</li>
</ul>
<h3 id="what-you-receive" tabindex="-1"><a class="header-anchor" href="#what-you-receive"><span>What You Receive</span></a></h3>
<p>The handler always receives an <a href="./incoming-event"><code v-pre>IncomingEvent</code></a> or a subclass of it.</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Which abstracts the raw request and wraps useful context like:</p>
<ul>
<li>Parameters</li>
<li>Headers</li>
<li>Payload/body</li>
...
<h3 id="what-happens-around-the-handler" tabindex="-1"><a class="header-anchor" href="#what-happens-around-the-handler"><span>What Happens Around the Handler</span></a></h3>
<p>Before and after your event handler is called, the system may:</p>
<ul>
<li>Execute middleware (e.g., authentication, logging, transformations)</li>
<li>Trigger lifecycle hooks (<code v-pre>onHandlingEvent</code>, <code v-pre>onEventHandled</code>, etc.)</li>
<li>Manage error propagation (<code v-pre>onExecutingErrorHandler</code>)</li>
<li>Build and finalize the response (<code v-pre>onPreparingResponse</code>, <code v-pre>onResponsePrepared</code>)</li>
</ul>
<p>This means you can focus purely on <strong>what the domain should do</strong>, while Stone.js handles the infrastructure logic transparently.</p>
...
<p>This is the <strong>simplest and most common</strong> way to register an event handler.</p>
<p>If your application is class-based and uses the <code v-pre>@StoneApp()</code> decorator, all you need to do is implement the <code v-pre>handle()</code> method:</p>
<div class="language-typescript line-numbers-mode" data-highlighter="prismjs" data-ext="ts"><pre v-pre><code><span class="line"><span class="token keyword">import</span> <span class="token punctuation">{</span> StoneApp<span class="token punctuation">,</span> IncomingEvent<span class="token punctuation">,</span> IEventHandler <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">"@stone-js/core"</span></span>
<span class="line"></span>
...
<span class="line"><span class="token keyword">export</span> <span class="token keyword">class</span> <span class="token class-name">Application</span> <span class="token keyword">implements</span> <span class="token class-name">IEventHandler<span class="token operator">&lt;</span>IncomingEvent<span class="token operator">></span></span> <span class="token punctuation">{</span></span>
<span class="line">  <span class="token function">handle</span><span class="token punctuation">(</span>event<span class="token operator">:</span> IncomingEvent<span class="token punctuation">)</span><span class="token operator">:</span> <span class="token keyword">void</span> <span class="token punctuation">{</span></span>
<span class="line">    <span class="token comment">// Do your logic here</span></span>
<span class="line">  <span class="token punctuation">}</span></span>
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>As soon as you decorate the class with <code v-pre>@StoneApp()</code>, the system recognizes it as your main application entrypoint and automatically wires up the event handler.</p>
<p>Note: You can only have <strong>one main event handler</strong> by default.<br>
For multiple handlers, see the <a href="../router">Router documentation</a> to route events conditionally.</p>
</template>
<template #tab1="{ value, isActive }">
...
<p>Prefer full control or want to register a function/factory manually?<br>
Use the <code v-pre>defineBlueprintConfig</code> function and set the <code v-pre>stone.kernel.eventHandler</code> key.</p>
<h4 id="function-based-handler" tabindex="-1"><a class="header-anchor" href="#function-based-handler"><span>Function-based Handler</span></a></h4>
...
<span class="line">  blueprint<span class="token punctuation">.</span><span class="token function">set</span><span class="token punctuation">(</span><span class="token string">'stone.kernel.eventHandler'</span><span class="token punctuation">,</span> <span class="token punctuation">{</span> module<span class="token operator">:</span> Application <span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="factory-based-handler" tabindex="-1"><a class="header-anchor" href="#factory-based-handler"><span>Factory-based Handler</span></a></h4>
<p>If you're using a factory shape, add <code v-pre>isFactory: true</code>:</p>
<div class="language-typescript line-numbers-mode" data-highlighter="prismjs" data-ext="ts"><pre v-pre><code><span class="line"><span class="token keyword">export</span> <span class="token keyword">const</span> AppBlueprint <span class="token operator">=</span> <span class="token function">defineBlueprintConfig</span><span class="token punctuation">(</span><span class="token punctuation">(</span>blueprint<span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">{</span></span>
<span class="line">  blueprint<span class="token punctuation">.</span><span class="token function">set</span><span class="token punctuation">(</span><span class="token string">'stone.kernel.eventHandler'</span><span class="token punctuation">,</span> <span class="token punctuation">{</span></span>
<span class="line">    module<span class="token operator">:</span> Application<span class="token punctuation">,</span></span>
...
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>This gives you low-level flexibility, useful for advanced bootstrapping or dynamic registration scenarios.</p>
</template>
</Tabs>
<p>In both modes, once registered, the framework ensures that the handler is executed for every <code v-pre>IncomingEvent</code> reaching your application.</p>
<h2 id="router-and-multi-route-support" tabindex="-1"><a class="header-anchor" href="#router-and-multi-route-support"><span>Router and Multi-Route Support</span></a></h2>
<p>By default, a Stone.js application has <strong>one event handler</strong>.
That’s perfect for nano-services, single-purpose FaaS functions, or CLI tools.
But what if your app needs to handle <strong>multiple types of requests</strong>, mapped to different logic? That’s where the <RouteLink to="/docs/router/"><strong>Router</strong></RouteLink> comes in.</p>
...
<ul>
<li>Multiple endpoints (e.g., <code v-pre>/users</code>, <code v-pre>/posts</code>)</li>
<li>Different event types (e.g., HTTP vs. WebSocket)</li>
<li>Modular handler definitions per domain</li>
</ul>
<p>…then install the <RouteLink to="/docs/router/"><strong>Router</strong></RouteLink> and use it to dispatch events to the appropriate handler.</p>
<p>The Router lives in the <strong>initialization dimension</strong>, just like event handlers.
It intercepts incoming events and <strong>routes them</strong> to the correct handler, based on metadata like path, method, or headers.</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Check out the <a href="../router">Router documentation</a> to learn more about how to set it up and use it.</p>
...
<ul>
<li>Created <strong>after</strong> the system is bootstrapped</li>
...
<li><strong>CLI</strong> commands</li>
<li><strong>WebSocket</strong> messages</li>
...
<ul>
<li><strong>Setup dimension</strong> tasks (e.g., registering services, adapters, middleware)</li>
<li><strong>Integration dimension</strong> operations (e.g., transforming raw input, mapping routes)</li>
</ul>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>The thinner your handler, the easier it is to test, maintain, and reuse.</p>
<h4 id="_2-treat-it-like-a-function-entry-point" tabindex="-1"><a class="header-anchor" href="#_2-treat-it-like-a-function-entry-point"><span>2. Treat It Like a Function Entry Point</span></a></h4>
<p>Think of your handler as a function signature for external input.</p>
<ul>
<li>Validate the event (if needed)</li>
<li>Transform the input (if needed)</li>
<li>Call into the domain</li>
<li>Return the output</li>
</ul>
<p>That’s it.<br>
No side quests, no side effects.</p>
<h4 id="_3-never-bind-to-context-specific-apis" tabindex="-1"><a class="header-anchor" href="#_3-never-bind-to-context-specific-apis"><span>3. Never Bind to Context-Specific APIs</span></a></h4>
<p>Avoid direct calls to <code v-pre>req</code>, <code v-pre>res</code>, <code v-pre>process</code>, <code v-pre>console</code>, or anything else tied to a specific environment.<br>
Use the <code v-pre>IncomingEvent</code> and domain services to stay agnostic and portable.</p>
<h4 id="_4-compose-with-middleware" tabindex="-1"><a class="header-anchor" href="#_4-compose-with-middleware"><span>4. Compose With Middleware</span></a></h4>
<p>If your handler is starting to grow:</p>
<ul>
<li>Move shared logic to middleware (auth, logging, headers)</li>
<li>Move domain logic to services or use cases</li>
<li>Let the handler just <strong>orchestrate</strong></li>
</ul>
<p>You’ll thank yourself when it’s time to scale or refactor.</p>
<h4 id="_5-prefer-class-shape-for-declarative-use" tabindex="-1"><a class="header-anchor" href="#_5-prefer-class-shape-for-declarative-use"><span>5. Prefer Class Shape for Declarative Use</span></a></h4>
<p>If you're using the declarative API and <code v-pre>@StoneApp()</code>, use the <strong>class-based handler</strong>.<br>
It integrates naturally with introspection, decorators, and tooling.</p>
<h4 id="_6-use-factories-for-container-access" tabindex="-1"><a class="header-anchor" href="#_6-use-factories-for-container-access"><span>6. Use Factories for Container Access</span></a></h4>
<p>If you need services or runtime config in your function-based handler, use the <strong>factory shape</strong>.<br>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>A clean handler is a scalable handler.<br>
...
<li>It is the <strong>only way</strong> to handle an <code v-pre>IncomingEvent</code></li>
<li>It can be written as a <strong>class</strong>, <strong>function</strong>, or <strong>factory</strong></li>
<li>It can be registered <strong>declaratively</strong> (with <code v-pre>@StoneApp</code>) or <strong>imperatively</strong> (via <code v-pre>defineBlueprintConfig</code>)</li>
<li>It runs <strong>once per incoming event</strong>, and may return a value, <code v-pre>void</code>, or a <code v-pre>Promise</code></li>
...
And absolutely zero assumptions about the platform.</p>
</blockquote>
<p>Ready to handle some events?</p>
</div></template>
